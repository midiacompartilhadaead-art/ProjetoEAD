...
$contentType   = isset($data['contentType']) ? trim($data['contentType']) : (substr($nomeArquivo, -4) === '.txt' ? 'text/plain' : 'application/octet-stream');
$valorTotal    = isset($data['valorTotal']) ? floatval($data['valorTotal']) : 0;
$itensDespesa  = isset($data['itensDespesa']) ? $data['itensDespesa'] : [];
...
function sanitizarNomeOneDrive($texto) {
...
$payload = [
    'modulo'        => strtoupper(sanitizarNomeOneDrive($modulo)),
    'nomePolo'      => strtoupper(sanitizarNomeOneDrive($nomePolo)),
